#!/usr/bin/env bash
#
# Record that the Rule 3 diagnostic has been agreed.
#
# Usage:
#   ca-diagnose-power/scripts/record-agreement.sh --client CLIENT \
...
#                                                                      "Stage 4: Rule 3 diagnostic agreed")
#
...
CMD=(uv run --project "$REPO_DIR" consultamatron decision record \
  ...
  --title "Stage 4: Rule 3 diagnostic agreed" \
  --field "Agreed=Rule 3 power diagnostic signed off")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
exec "${CMD[@]}"
