#!/usr/bin/env bash
#
# Record that the Rule 4 diagnostic has been agreed.
#
# Usage:
#   ca-diagnose-future/scripts/record-agreement.sh --client CLIENT \
...
#                                                                      "Stage 5: Rule 4 diagnostic agreed")
#
...
CMD=(uv run --project "$REPO_DIR" consultamatron decision record \
  ...
  --title "Stage 5: Rule 4 diagnostic agreed" \
  --field "Agreed=Rule 4 shadow of the future diagnostic signed off")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
exec "${CMD[@]}"
