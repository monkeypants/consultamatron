#!/usr/bin/env bash
#
# Record that the aggregated diagnostics have been agreed.
#
# Usage:
#   ca-aggregate/scripts/record-agreement.sh --client CLIENT \
#     --engagement ENGAGEMENT --project PROJECT \
#     --field "Hotspots=..." --field "Root causes=..."
#
# Files modified by this script:
#   clients/{client}/engagements/{engagement}/{slug}/decisions.json — Decision log (append:
#                                                                      "Stage 8: Diagnostics aggregated and agreed")
#
...
CMD=(uv run --project "$REPO_DIR" consultamatron decision record \
  ...
  --title "Stage 8: Diagnostics aggregated and agreed" \
  --field "Agreed=Aggregated diagnostic picture signed off by client")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
exec "${CMD[@]}"
