#!/usr/bin/env bash
#
# Record that the Rule 6 diagnostic has been agreed.
#
# Usage:
#   ca-diagnose-rewards/scripts/record-agreement.sh --client CLIENT \
...
#                                                                      "Stage 7: Rule 6 diagnostic agreed")
#
...
CMD=(uv run --project "$REPO_DIR" consultamatron decision record \
  ...
  --title "Stage 7: Rule 6 diagnostic agreed" \
  --field "Agreed=Rule 6 rewards diagnostic signed off")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
exec "${CMD[@]}"
