#!/usr/bin/env bash
#
# Record that the Rule 5 diagnostic has been agreed.
#
# Usage:
#   ca-diagnose-reciprocity/scripts/record-agreement.sh --client CLIENT \
...
#                                                                      "Stage 6: Rule 5 diagnostic agreed")
#
...
CMD=(uv run --project "$REPO_DIR" consultamatron decision record \
  ...
  --title "Stage 6: Rule 5 diagnostic agreed" \
  --field "Agreed=Rule 5 reciprocity diagnostic signed off")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
exec "${CMD[@]}"
