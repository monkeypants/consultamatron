#!/usr/bin/env bash
#
# Record that the Rule 1 diagnostic has been agreed.
#
# Usage:
#   ca-diagnose-understanding/scripts/record-agreement.sh --client CLIENT \
...
#                                                                      "Stage 2: Rule 1 diagnostic agreed")
#
...
CMD=(uv run --project "$REPO_DIR" consultamatron decision record \
  ...
  --title "Stage 2: Rule 1 diagnostic agreed" \
  --field "Agreed=Rule 1 understanding diagnostic signed off")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
exec "${CMD[@]}"
