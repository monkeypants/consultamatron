...
Key constraint: Protected Variations — voice layer is a pure transform, no side effects on analytical artifacts. Gate artifacts remain analytical; voice output is derivative.

Subsequent deliberation (pedagogic-gradient-descent) refined recommendation: voice selection should be a function of concept gap, evidence level, and operator preference — not blanket preference.

Includes in-character explanations from Feynman (same physics, different lecture hall), Garden (bedside manner vs diagnosis), and Krieger (delivery mechanism vs compound).
