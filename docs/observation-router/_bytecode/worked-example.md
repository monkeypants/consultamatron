...
Agent extracts three observations from one operator remark about segment rejection: client observation (sponsor procurement speed criterion → workspace), project observation (political not structural constraint → canvas-1 + maps-1), methodology observation (BMC should distinguish "bad fit" from "client constraint" → personal/ skillset). Each routed to destinations whose needs it serves.

Open questions surfaced: instance-level needs authoring (operator vs agent vs both), brief size at scale, observation granularity (one per destination or one with fan-out).
