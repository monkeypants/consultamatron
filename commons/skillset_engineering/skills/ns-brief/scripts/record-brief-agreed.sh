#!/usr/bin/env bash
#
# Record that the new-skillset brief has been agreed.
#
# Usage:
...
#   clients/{client}/engagements/{engagement}/{slug}/decisions.json
#   clients/{client}/engagements/{engagement}/projects.json
...
CMD=($CLI decision record \
  --client "$CLIENT" --engagement "$ENGAGEMENT" --project "$PROJECT" \
  --title "Stage 1: Skillset brief agreed" \
  --field "Agreed=New skillset scope signed off by operator")
for f in "${FIELDS[@]}"; do
  CMD+=(--field "$f")
done
"${CMD[@]}"
