...
#
# Usage:
#   engage/scripts/register-project.sh --client CLIENT --slug SLUG \
#     --skillset SKILLSET --scope "SCOPE" [--notes "NOTES"]
#
# Files modified by this script:
#   clients/{client}/projects/index.json             — Project registry (append)
#   clients/{client}/projects/{slug}/decisions.json  — Decision log (created with
...
CLIENT="" SLUG="" SKILLSET="" SCOPE="" NOTES=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
if [[ -z "$CLIENT" || -z "$SLUG" || -z "$SKILLSET" || -z "$SCOPE" ]]; then
  echo "Usage: $0 --client CLIENT --slug SLUG --skillset SKILLSET --scope SCOPE [--notes NOTES]" >&2
  exit 1
fi
